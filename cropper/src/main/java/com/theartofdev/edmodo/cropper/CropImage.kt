...
/**
 * Helper to simplify crop image work like starting pick-image acitvity and handling camera/adapter intents.<br></br>
...
object CropImage {
     ...
     * applications for the device like camera (MyCamera), galery (Photos), store apps (Dropbox), etc.<br></br>
    ...
    /**
     * Same as [startPickImageActivity][.startPickImageActivity] method but instead
     ...
     * The source can be camera's  (ACTION_IMAGE_CAPTURE) or adapter's (ACTION_GET_CONTENT).<br></br>
     ...
     * If the outputFileUri is null, a default Uri will be created with [.getCaptureImageOutputUri], so
     * then
     * you will be able to get the pictureUri using [.getPickImageResultUri]. Otherwise, it is
     * just you use
 ...
 * question](http://stackoverflow.com/questions/32789027/android-m-camera-intent-permission-bug).
    ...
    fun isExplicitCameraPermissionRequired(context: Context): Boolean {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
                hasPermissionInManifest(context, "android.permission.CAMERA") && context.checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED
    }
     ...
     */
    fun isReadExternalStoragePermissionsRequired(context: Context, uri: Uri): Boolean {
    ...
    /**
     * Create [ActivityBuilder] instance to open image picker for cropping and then start
     * [CropImageActivity] to crop the selected image.<br></br>
     * Result will be received in [Activity.onActivityResult] and can be retrieved
    ...
    /**
     * Create [ActivityBuilder] instance to start [CropImageActivity] to crop the given image.<br></br>
     * Result will be received in [Activity.onActivityResult] and can be retrieved
     * using [.getActivityResult].
     *
    ...
    class ActivityBuilder(
        ...
        fun getIntent(context: Context, cls: Class<*>?): Intent {
            ...
            intent.putExtra(CropImageOptions.Companion.BUNDLE_KEY, bundle)
            return intent
        }
        ...
        fun start(activity: Fragment) {
            mOptions.validate()
            activity.startActivityForResult(getIntent(activity.activity!!), CROP_IMAGE_ACTIVITY_REQUEST_CODE)
        }
         ...
         * @param fragment fragment to receive result
         */
        fun start(context: Context, fragment: Fragment, cls: Class<*>?) {
         ...
         */
        @RequiresApi(api = Build.VERSION_CODES.HONEYCOMB)
        fun start(context: Context, fragment: android.app.Fragment, cls: Class<*>?) {
            fragment.startActivityForResult(getIntent(context, cls), CROP_IMAGE_ACTIVITY_REQUEST_CODE)
        }
        ...
        /**
         * The initial crop window padding from image borders in percentage of the cropping image dimensions.<br></br>
         * *Default: 0.1*
         */
        ...
        /**
         * the color of the overlay background around the crop window cover the image parts not in the crop window.<br></br>
         ...
         * *Default: 0, 0 - not set, will not resize*
         */
        fun setRequestedSize(reqWidth: Int, reqHeight: Int, options: RequestSizeOptions?): ActivityBuilder {
         ...
         * Used if you want to crop the image manually and need only the crop rectangle and rotation data.<br></br>
         * *Default: false*
         */
        ...
        /**
         * the initial rotation to set on the cropping image after loading (0-360 degrees clockwise).<br></br>
    ...
    class ActivityResult : CropResult, Parcelable {
        constructor(originalUri: Uri?, uri: Uri?, error: Exception?,
                    cropPoints: FloatArray?, cropRect: Rect?, rotation: Int, wholeImageRect: Rect?, sampleSize: Int) : super(null, originalUri, null, uri, error, cropPoints, cropRect, wholeImageRect, rotation, sampleSize) {
        }
